...
PY
  ...
  SIMULATOR_DEVICE_DESTINATION="platform=iOS Simulator,arch=${SIMULATOR_ARCH},id=${SIM_UDID}"
}
...
function assert_xcresult_executed_tests() {
  local result_bundle_path="$1"
  local minimum_test_count="${2:-1}"

  python3 - "$result_bundle_path" "$minimum_test_count" <<'PY'
import json
import subprocess
import sys
...
minimum_test_count = int(sys.argv[2])

if not path.exists():
    raise SystemExit(f"Missing xcresult bundle: {path}")
...
def load_summary_metrics() -> dict:
    ...
    return {
        "testsCount": payload.get("totalTestCount"),
        "testsSkippedCount": payload.get("skippedTests"),
    }

def load_action_metrics() -> dict:
    completed = subprocess.run(
        ["xcrun", "xcresulttool", "get", "object", "--legacy", "--path", str(path), "--format", "json"],
        check=False,
        capture_output=True,
        text=True,
    )
    if completed.returncode != 0:
        raise SystemExit(completed.stderr.strip() or f"Failed to inspect xcresult: {path}")
    payload = json.loads(completed.stdout)
    actions = payload.get("actions", {}).get("_values", [])
    if not actions:
        return {}
    return actions[0].get("actionResult", {}).get("metrics", {})
...
tests_count = metric_value(summary_metrics.get("testsCount"))
if tests_count is None:
    tests_count = metric_value(action_metrics.get("testsCount"))
if tests_count is None or int(tests_count) < minimum_test_count:
    raise SystemExit(
        f"{path}: expected at least {minimum_test_count} executed test(s), found {tests_count!r}"
    )

tests_skipped = metric_value(summary_metrics.get("testsSkippedCount"))
if tests_skipped not in (None, 0):
    raise SystemExit(f"{path}: skipped tests detected ({tests_skipped})")
PY
}
...
function run_infra_safety_report() {
  ...
  signpost_count=$(grep -ri 'OSSignposter\|signposter\.beginInterval\|signposter\.endInterval' \
...
function run_checked_xcodebuild_impl() {
  ...
  if [[ "${command[0]}" == "xcodebuild" ]]; then
    ...
    command+=("SWIFT_SUPPRESS_WARNINGS=NO" "GCC_WARN_INHIBIT_ALL_WARNINGS=NO")
  fi
...
function gate_app_tests() {
  ...
  local tests_dir="$ROOT_DIR/modules/native-chat/Tests/NativeChatTests"
  if [[ ! -d "$tests_dir" ]] || ! find "$tests_dir" -maxdepth 1 -name '*.swift' -print -quit | grep -q .; then
    echo "No app unit tests are currently defined." >&2
    return 0
  fi
  ...
  run_checked_xcodebuild glassgpt-unit-tests \
    ...
    test
  assert_xcresult_executed_tests "$CI_OUTPUT_DIR/GlassGPTUnitTests.xcresult"
}
...
function gate_package_tests() {
  log "Running package logic tests"
  local result_bundle_path="$CI_OUTPUT_DIR/NativeChatPackageTests.xcresult"

  force_remove_path "$result_bundle_path"
  run_checked_xcodebuild_in_dir nativechat-package-tests "$ROOT_DIR/modules/native-chat" \
    xcodebuild \
    -scheme NativeChat-Package \
    -clonedSourcePackagesDirPath "$CI_SOURCE_PACKAGES_DIR" \
    -destination "$SIMULATOR_DEVICE_DESTINATION" \
...
function gate_coverage_report() {
  ...
  if [[ -d "$CI_OUTPUT_DIR/NativeChatPackageTests.xcresult" ]]; then
    coverage_sources+=("$CI_OUTPUT_DIR/NativeChatPackageTests.xcresult")
  fi

  if [[ -d "$CI_OUTPUT_DIR/NativeChatArchitectureTests.xcresult" ]]; then
    coverage_sources+=("$CI_OUTPUT_DIR/NativeChatArchitectureTests.xcresult")
  fi
...
function gate_reinstall_compatibility() {
  ...
  if (( ${#REINSTALL_UI_TEST_CASES[@]} == 0 )); then
    echo "No reinstall compatibility UI cases are defined for the current product surface." >&2
    exit 1
  fi
...
function assert_release_readiness() {
  ...
  if ! search_quiet "stable-4.12" "$ROOT_DIR/docs/branch-strategy.md"; then
    echo "branch-strategy.md must retain the frozen 4.12 rollback line." >&2
    exit 1
  fi
  ...
  if ! search_quiet "feature/\\*\\*" "$ROOT_DIR/.github/workflows/ios.yml"; then
    echo "ios.yml must include feature branch coverage." >&2
    exit 1
  fi

  if ! search_quiet "codex/feature/\\*\\*" "$ROOT_DIR/.github/workflows/ios.yml"; then
    echo "ios.yml must include mirrored feature branch coverage." >&2
    exit 1
  fi
...
if [[ "${PRESERVE_CI_OUTPUTS:-0}" != "1" ]] \
  && (( ${#requested_gates[@]} > 1 )) \
  && [[ "${requested_gates[0]}" != "coverage-report" ]]; then
  clean_outputs
fi

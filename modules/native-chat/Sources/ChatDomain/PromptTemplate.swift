...
public enum BuiltInPromptTemplates {
    ...
    public static let translator = PromptTemplateDescriptor(
        name: "Translator",
        systemPrompt: "You are a professional translator. Translate the user's text accurately, preserving tone and nuance. If no target language is specified, translate to English. Provide only the translation without explanations unless asked.",
        isBuiltIn: true
    )
    ...
    public static let codeReviewer = PromptTemplateDescriptor(
        name: "Code Reviewer",
        systemPrompt: "You are a senior software engineer performing a thorough code review. Focus on correctness, performance, security, readability, and maintainability. Provide specific, actionable feedback with examples. Highlight both issues and well-written code.",
        isBuiltIn: true
    )
    ...
    public static let writingAssistant = PromptTemplateDescriptor(
        ...
        systemPrompt: "You are a professional writing assistant. Help the user improve their writing by focusing on clarity, grammar, tone, and structure. Provide suggestions while preserving the author's voice and intent.",
        isBuiltIn: true
    )

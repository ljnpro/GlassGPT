...
enum BackendNetworkLogger {
    ...
    static func logResponse(
        ...
        let rid = requestId.map { " rid=\($0)" } ?? ""
        let msg = "[HTTP] \(method) \(path) → \(statusCode) (\(elapsedMs)ms)\(rid)"
        networkLogger.debug("\(msg, privacy: .public)")
    }

    static func logError(method: String, path: String, error: any Error, requestId: String? = nil) {
        let sanitized = sanitizeError(error)
        let rid = requestId.map { " rid=\($0)" } ?? ""
        let msg = "[HTTP] \(method) \(path) failed: \(sanitized)\(rid)"
        networkLogger.error("\(msg, privacy: .public)")
    }

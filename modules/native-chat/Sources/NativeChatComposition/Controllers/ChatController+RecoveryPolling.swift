...
extension ChatController {
    func pollResponseUntilTerminal(session: ReplySession, responseId: String) async {
                ...
                case .completed, .incomplete, .failed, .unknown:
                    if let message = findMessage(byId: session.messageID) {
                        if result.status == .failed || result.status == .incomplete,
                           visibleSessionMessageID == session.messageID {
        ...
        guard !Task.isCancelled, let message = findMessage(byId: session.messageID) else { return }
            ...
            case .queued, .inProgress:
                if let message = findMessage(byId: messageId),
                   let session = makeRecoverySession(for: message) {
                    registerSession(session, execution: SessionExecutionState(service: serviceFactory()), visible: false)

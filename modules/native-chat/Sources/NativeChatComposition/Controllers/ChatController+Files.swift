...
extension ChatController {
    ...
    func handleSandboxLinkTap(message: Message, sandboxURL: String, annotation: FilePathAnnotation?) {
        ...
        Task { @MainActor in
            do {
                if let annotation,
                   let cachedResource = await fileDownloadService.cachedGeneratedFile(
                ...
                let resource = try await fileDownloadService.prefetchGeneratedFile(
        ...
        Task { @MainActor in
            ...
            Task.detached(priority: .utility) {
                for annotation in annotationsToPrefetch {
                    do {
                        _ = try await self.fileDownloadService.prefetchGeneratedFile(
                            fileId: annotation.fileId,
                            containerId: annotation.containerId,

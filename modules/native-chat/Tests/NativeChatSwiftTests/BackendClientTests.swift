...
struct BackendClientTests {
    ...
    func `connection check uses session authorization`() async throws {
        ...
        #expect(
            RecordingBackendURLProtocol.state.snapshot.recordedRequests.last?.appVersionHeader == "5.7.0"
        )
        #expect(response.backendVersion == "5.7.0")
    ...
    func `expired session refresh persists updated session`() async throws {
        ...
        #expect(recorded.last?.appVersionHeader == "5.7.0")

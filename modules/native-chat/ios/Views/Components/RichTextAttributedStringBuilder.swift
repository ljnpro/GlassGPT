...
enum RichTextAttributedStringBuilder {
    ...
    static func parseRichText(_ text: String) -> AttributedString {
        parse(text, style: bodyStyle, preserveLinksFromAppleParser: true)
    }

    static func parseStreamingText(_ text: String) -> AttributedString {
        parse(text, style: bodyStyle, preserveLinksFromAppleParser: false)
    }

    static func parseThinkingText(_ text: String) -> AttributedString {
    ...
    private static func parsedMarkdownText(_ text: String) -> AttributedString? {
        do {
            ...
            Loggers.app.debug("[RichTextAttributedStringBuilder] Apple markdown parse failed: \(error.localizedDescription)")
            return nil
        }
    }

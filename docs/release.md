...
- default branch: `main`
- active stable branch: `codex/stable-4.11`
- frozen prior stable branch: `codex/stable-4.10`
...
```bash
./scripts/release_testflight.sh 4.11.0 20197 --branch codex/stable-4.11
```
...
```bash
./scripts/release_testflight.sh 4.11.0 20197 --branch codex/stable-4.11 --preserve-main-as codex/stable-4.10 --force-main-with-lease
```
...
```bash
./scripts/release_testflight.sh 4.11.0 20197 --branch codex/stable-4.11 --preflight-only
```
...
4. any release-specific `4.11.0` scoring or verification scripts
5. manual parity checklist from `docs/parity-baseline.md`
6. clean worktree on `codex/stable-4.11`
7. version/build match the intended `4.11.0 (20197 or next available)` candidate
8. Phase G has been re-evaluated and the ADR updated before release
9. workflow triggers and readiness docs all reference `codex/stable-4.11`
...
2. verify `codex/stable-4.11` points to the release commit
3. verify `v4.11.0` points to the same commit
4. verify `git ls-remote` shows `codex/stable-4.11`, `main`, `codex/stable-4.10`, and `v<marketing-version>` aligned
